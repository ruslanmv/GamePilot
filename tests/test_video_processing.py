...
cv2 = pytest.importorskip("cv2", reason="opencv-python not installed (optional 'vision' extra)")
np = pytest.importorskip("numpy")

# Helper script lives outside the installed package; only run when present.
_SCRIPTS_DIR = os.path.join(os.path.dirname(__file__), "..", "scripts")
if not os.path.isfile(os.path.join(_SCRIPTS_DIR, "video_processing.py")):
    pytest.skip("scripts/video_processing.py not available", allow_module_level=True)

sys.path.insert(0, _SCRIPTS_DIR)

from video_processing import extract_frames  # noqa: E402
